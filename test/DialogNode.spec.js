...
let fake_child1_getData = sinon.stub().returns({uuid:'child1',class:'TalkNode'});
let fake_child1 = {getId:fake_child1_getId,getData:fake_child1_getData};

let fake_child2_getId = sinon.stub().returns('child2');
let fake_child2_getData = sinon.stub().returns({uuid:'child2',class:'TalkNode'});
let fake_child2 = {getId:fake_child2_getId,getData:fake_child2_getData};

let fake_child3_getId = sinon.stub().returns('child3');
let fake_child3_getData = sinon.stub().returns({uuid:'child3',class:'TalkNode'});
let fake_child3 = {getId:fake_child3_getId,getData:fake_child3_getData};

let fake_child4_getId = sinon.stub().returns('child4');
let fake_child4_getData = sinon.stub().returns({uuid:'child4',class:'TalkNode'});
let fake_child4 = {getId:fake_child4_getId,getData:fake_child4_getData};

let fake_addNode = sinon.stub().returns(null);
fake_addNode.withArgs({uuid:'child1',class:'TalkNode'}).returns(fake_child1);
fake_addNode.withArgs({uuid:'child2',class:'TalkNode'}).returns(fake_child2);
fake_addNode.withArgs({uuid:'child3',class:'TalkNode'}).returns(fake_child3);
fake_addNode.withArgs({uuid:'child4',class:'TalkNode'}).returns(fake_child4);
...
describe('Given an instance of DialogNode',function () {
  ...
  describe('#setNodeAsChild()',function() {
    it('should treat node as child', () => {
      ...
      expect(instance.getChildren().length).to.equal(1);
    });
    it('should set node as start talk if it is the first child', () => {
      instance.setNodeAsChild(fake_child4);
      expect(instance.getStartTalk()).to.equal('child4');
    });
  });
  describe('#addChild()',function() {
    it('should add new child to node and set startTalk if it is first child', () => {
      expect(instance.addChild({uuid:'child1',class:'TalkNode'}).getId()).to.equal('child1');
      expect(instance.addChild({uuid:'child2',class:'TalkNode'}).getId()).to.equal('child2');
      expect(instance.getStartTalk()).to.equal('child1');
    });
  });
  describe('#removeChild()',function() {
    it('should remove child and set startTalk to first available child', () => {
      instance.addChild({uuid:'child1',class:'TalkNode'});
      instance.addChild({uuid:'child2',class:'TalkNode'});
      expect(instance.getStartTalk()).to.equal('child1');
  ...
  describe('#setStartTalk()',function() {
    it('should set talk node id from which dialog starts', () => {
      instance.addChild({uuid:'child1',class:'TalkNode'});
      instance.addChild({uuid:'child2',class:'TalkNode'});
      instance.addChild({uuid:'child3',class:'TalkNode'});
